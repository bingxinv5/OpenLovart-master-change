...
function resolveLodQuality(sourceMime: string, level: number, qualities?: Record<number, number>): number {
...
function chooseLodEncodeType(sourceMime: string): 'image/webp' | 'image/jpeg' {
  return sourceMime.startsWith('image/png') ? 'image/webp' : 'image/jpeg';
}

async function encodeLodBlob(
