                    ...
                    <div className="rounded-2xl border border-slate-200/70 bg-white shadow-sm">
                        ...
                        <div className={`${referencePreviewItems.length > 0 ? 'group/refs' : ''} relative px-3 pb-2.5`}>
                            <div className="relative" style={{ minHeight: '32px' }}>
                                ...
                                <div className={`flex items-end gap-1 transition-all duration-300 ease-out ${referencePreviewItems.length > 0 ? 'group-hover/refs:opacity-0 group-hover/refs:scale-95 group-hover/refs:pointer-events-none' : ''}`}>
                                ...
                                {referencePreviewItems.length > 0 && (
                                    <div className="absolute inset-0 flex items-end gap-1.5 transition-all duration-300 ease-out opacity-0 scale-95 pointer-events-none group-hover/refs:opacity-100 group-hover/refs:scale-100 group-hover/refs:pointer-events-auto">
                                        <button type="button" onClick={() => { if (confirmClear) { clearMountedReferences(); setConfirmClear(false); } else { setConfirmClear(true); setTimeout(() => setConfirmClear(false), 2000); } }} className={`shrink-0 self-center rounded-full p-1 transition-colors ${confirmClear ? 'bg-rose-50 text-rose-500 ring-1 ring-rose-200' : 'text-slate-300 hover:text-slate-500'}`} title={confirmClear ? '再次点击确认清空' : '清空素材'}>
                                            <X size={14} />
                                        </button>
