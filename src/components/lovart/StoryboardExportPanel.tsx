...
import { AlertCircle, ArrowLeft, ArrowRight, ArrowUpDown, Download, LayoutGrid, Loader2, Type, X } from 'lucide-react';
import type { StoryboardExportOptions, StoryboardCaptionMode, StoryboardExportStyle } from '@/lib/storyboard-export';
...
import { DEFAULT_EXPORT_OPTIONS, loadStoryboardExportOptions, persistStoryboardExportOptions } from '@/lib/storyboard-export-settings';
import {
    type StoryboardExportPreviewItem,
    type StoryboardOrderSource,
    type StoryboardOrderStatus,
    type StoryboardFieldValidation,
    type BatchMetadataFields,
    type BatchApplyMode,
    type BatchShotCodeFields,
    type StoryboardPreflightSummary,
    captionOptions,
    exportStyleOptions,
    DEFAULT_BATCH_METADATA_FIELDS,
    DEFAULT_BATCH_SHOT_CODE_FIELDS,
    getOrderStatusMeta,
    getCaptionOptionLabel,
    getTemplateSummary,
    getCaptionByMode,
...
const DEFAULT_OPTIONS = DEFAULT_EXPORT_OPTIONS;
...
export function StoryboardExportPanel({
  ...
  }, [options.backgroundColor, options.captionMode, options.columns, options.exportStyle, options.gap, options.headerSubtitle, options.headerTitle, options.lockCurrentOrder, options.padding, options.showHeader, options.showNumbers, options.textColor]);
...
;
  ...
  const previewTheme = useMemo(() => resolvePreviewTheme(options.exportStyle, options.backgroundColor), [options.backgroundColor, options.exportStyle]);
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 rounded-2xl border border-slate-200/60 bg-slate-50/60 p-3">
        <div className="mb-2 flex items-center justify-between text-[11px] font-medium text-slate-500">
          <span>导出预览</span>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={restoreSelectionOrder}
            ...
            <button
              type="button"
              onClick={handleAutofillMissingShotCodes}
              disabled={!!options.lockCurrentOrder}
            ...
            <button
              type="button"
              onClick={applyStoryboardShotOrder}
              disabled={!!options.lockCurrentOrder}
        ...
        <div
          className="rounded-xl border border-white/80 p-3"
          style={{ backgroundColor: previewTheme.panelBg }}
        >
          ...
          >
            {previewItems.map((item, index) => {
              ...
              return (
                <div key={item.id} className={`overflow-hidden rounded-[18px] border shadow-sm transition-all ${resolvedActiveItemId === item.id ? 'border-sky-300 shadow-[0_0_0_1px_rgba(125,211,252,0.5),0_12px_32px_rgba(14,165,233,0.16)]' : ''} ${validationById.has(item.id) ? 'ring-1 ring-rose-200' : ''}`} style={{ backgroundColor: previewTheme.cardBg, borderColor: validationById.has(item.id) ? '#fda4af' : resolvedActiveItemId === item.id ? undefined : previewTheme.cardBorder }}>
                  {isStoryboardMetaMode && (
                    <div className="flex items-center justify-between gap-2 px-3 py-2" style={{ backgroundColor: previewTheme.headerBg }}>
                  ...
                  {isStoryboardMetaMode ? (
                    <div className="space-y-2 border-t border-slate-100 bg-gradient-to-br from-white via-slate-50 to-amber-50/60 px-3 py-3">
                      ...
                      <div className={`rounded-xl border px-2.5 py-2 ${previewTheme.noteBg} ${previewTheme.noteBorder}`}>
                        ...
                        <div className="mt-1 line-clamp-2 text-[11px] leading-5" style={{ color: options.textColor }}>
                          {metaNote}
                        </div>
                      </div>
                    </div>
                  ) : options.captionMode !== 'none' && (
                    <div className="px-3 py-2 text-[11px]" style={{ color: options.textColor }}>
