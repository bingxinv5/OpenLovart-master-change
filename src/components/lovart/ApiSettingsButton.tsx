    ...
    return (
        <>
            <button
                ...
                className={`canvas-settings-trigger relative flex h-8 w-8 items-center justify-center rounded-full transition-colors ${hasCustomConfig ? 'canvas-settings-trigger-alert' : ''}`}
                title="设置中心"
            >
                <Settings size={16} />
                {hasCustomConfig && (
                    <span className="canvas-settings-indicator absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 bg-green-500" />
                )}
            </button>
