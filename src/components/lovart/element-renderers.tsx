...
export function ImageGeneratorElementRenderer({ el, isGeneratorSubmitting }: { el: CanvasElement; isGeneratorSubmitting: boolean }) {
    ...
    return (
        <div className={`canvas-tool-node canvas-tool-node-image w-full h-full border-2 rounded-xl flex items-center justify-center ${isBusy ? 'is-busy' : ''}`}>
            {isBusy ? (
                <div className="canvas-tool-node-spinner animate-spin rounded-full h-12 w-12 border-4" />
            ) : (
                <div className="canvas-tool-node-icon flex h-20 w-20 items-center justify-center rounded-[22px] opacity-80"><svg viewBox="0 0 24 24" fill="currentColor" className="h-12 w-12"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z" /></svg></div>
            )}
        </div>
    );
}
    ...
    return (
        <div className={`canvas-tool-node canvas-tool-node-storyboard h-full w-full rounded-xl border-2 ${el.generatingTaskId || isGeneratorSubmitting ? 'is-busy' : el.generatingError ? 'is-error' : ''}`}>
            <div className="flex h-full flex-col items-center justify-center px-6 text-center">
                {isBusy ? (
                    <div className="canvas-tool-node-icon flex h-20 w-20 items-center justify-center rounded-[26px]">
                    ...
                    <div className="canvas-tool-node-icon flex h-20 w-20 items-center justify-center rounded-[26px]"><svg viewBox="0 0 24 24" fill="currentColor" className="h-10 w-10"><path d="M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1zm2 3v2h2V8H6zm0 4v2h2v-2H6zm0 4v1h12v-1H6zm4-8v2h8V8h-8zm0 4v2h8v-2h-8z" /></svg></div>
                )}
            </div>
        </div>
    );
...
export function VideoGeneratorElementRenderer({ el, isGeneratorSubmitting }: { el: CanvasElement; isGeneratorSubmitting: boolean }) {
    ...
    return (
        <div className={`canvas-tool-node canvas-tool-node-video w-full h-full border-2 rounded-xl flex items-center justify-center ${isBusy ? 'is-busy' : ''}`}>
            {isBusy ? (
                <div className="canvas-tool-node-spinner animate-spin rounded-full h-12 w-12 border-4" />
            ) : (
                <div className="canvas-tool-node-icon flex h-20 w-20 items-center justify-center rounded-[22px] opacity-80"><svg viewBox="0 0 24 24" fill="currentColor" className="h-12 w-12"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z" /></svg></div>
            )}
        </div>
    );
}
